...
{
    public partial class FormThemmoiBom : Form
    {
        ...
        public FormThemmoiBom(int i, int main_id, string field_name, string table_name)
        {
            id_BSKQ = i;
        ...
        private void LoadCBStaff(ComboBox cb)
        {
            ...
            //{
            UtilsDatabase.buildCombobox(cb, "SELECT dept_id_web, type_name FROM cecm_bomb", "dept_id_web", "type_name");
            //}
        ...
        private bool UpdateInfomation()
        {
            try
            {
                ...
                {
                    // Chua co tao moi
                    SqlCommand cmd2 = new SqlCommand("INSERT INTO [dbo].[groundDeliveryRecordsMember] ([table_name],[field_name],[main_id],[long1],[string1],[string2],[string3],[string4],[string5]) Values (@table_name,@field_name,@main_id,@long1,@string1,@string2,@string3,@string4,@string5)", _Cn);
                    ...
                    long1.Value = txtString1.SelectedValue;
                ...
                {
                    // Chua co tao moi
                    SqlCommand cmd2 = new SqlCommand("UPDATE [dbo].[groundDeliveryRecordsMember] SET table_name = @table_name, field_name=@field_name, main_id=@main_id,long1=@long1,string1=@string1,string2=@string2,string3=@string3,string4=@string4,string5=@string5 WHERE gid =  " + id_BSKQ, _Cn);
            ...
            if (i != 0)
            {
                ...
                if (datatable.Rows.Count != 0)
                {
                    foreach (DataRow dr in datatable.Rows)
                    {
                        ...
                        if (long.TryParse(dr["long1"].ToString(), out long command_id))
                        {
                            txtString1.SelectedValue = command_id;
                        }
        ...
        {
            if(!(txtString1.SelectedValue is long))
            {
                return;
            }
            if ((long)txtString1.SelectedValue <= 0)
            {
                e.Cancel = true;
                //comboBox_TenDA.Focus();
                errorProvider1.SetError(txtString1, "Chưa chọn loại bom mìn");
                return;
            }
            else
            {
