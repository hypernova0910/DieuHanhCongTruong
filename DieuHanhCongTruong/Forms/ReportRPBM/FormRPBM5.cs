...
{
    public partial class FormRPBM5 : Form
    {
        private SqlConnection cn = null;
        public string ipAddr = "", databaseName = "", userName = "";
        public int idDuan;
        public string table_name = "GROUNDCONSTRUCTRECORDS";
        public string field_name1 = "GroundConstructRecords_GroundConstructRecords_CDTMember";
        public string field_name2 = "GroundConstructRecords_GroundConstructRecords_MonitorMem";
        public string field_name3 = "GroundConstructRecords_GroundConstructRecords_ConstuctMem";
        public FormRPBM5()
        {
        ...
        private void LoadData(string name, string date1, string date2)
        {
            try
            {
                ...
                sqlAdapter = new SqlDataAdapter(string.Format("select gid as 'ID', cecm_program_idST as 'Dự án', datesST as 'Thời gian', address as 'Địa điểm' from [dbo].[RPBM5] where cecm_program_idST LIKE N'%" + name + "%' and datesST >'" + date1 + "' and datesST <'" + date2 + "'"), cn);
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 4)
            {
                ...
                string sourceFile = AppUtils.GetAppDataPath() + "\\TempExcel\\BB_RPBM5.xls";
                string pathFile = "";
                saveFileDialog1.FileName = "RPBM5_" + Guid.NewGuid().ToString();
                ...
                if (pathFile != null)
                {
                    using (ExcelEngine excelEngine = new ExcelEngine())
                    {
                        ...
                        SqlDataAdapter sqlAdapterProvince = new SqlDataAdapter(
                            ...
                            "FROM RPBM5  " +
                            "left join cecm_provinces Tinh on RPBM5.province_id = Tinh.id " +
                            "left join cecm_programData on cecm_programData.id = RPBM5.cecm_program_id " +
                        ...
                        if (datatable.Rows.Count != 0)
                        {
                            ...
                            marker.AddVariable("homnay", "Hôm nay, ngày " + dr["time_signedST"].ToString() + "tại hiện trường mặt bằng dự án: " + dr["cecm_program_idST"].ToString());
                            marker.AddVariable("bienban", "Biên bản được lập thành " + dr["amount"].ToString() + " bản lưu giữ tại hồ sơ hoàn công công trình.");
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                SqlDataAdapter sqlAdapterProvince = new SqlDataAdapter("SELECT * , cecm_programData.code as 'cecm_program_code'  FROM RPBM5 left join cecm_programData on cecm_programData.id = RPBM5.cecm_program_id where gid = " + id_kqks, cn);
                ...
                foreach (DataRow dr in datatableProvince.Rows)
                {
                    ...
                    lst.groundConstrucRecords_CDTMember_lstSubTable = Lst1;
                    lst.groundConstrucRecords_MonitoMem_lstSubTable = Lst2;
                    lst.groundConstrucRecords_ConsMem_lstSubTable = Lst3;
                    string json = JsonConvert.SerializeObject(lst, Formatting.Indented);
                    var saveLocation = AppUtils.SaveFileJson("JsonRPBM5");
            ...
            if (e.ColumnIndex == 6)
            {
                ...
                FrmThemmoiRPBM5 frm = new FrmThemmoiRPBM5(id_kqks);
                frm.Text = "CHỈNH SỬA BIÊN BẢN BÀN GIAO MẶT BẰNG CHO THI CÔNG";
                frm.ShowDialog();
                LoadData(TxtTImkiem.Text, timeNgaybatdau.Text, timeNgayketthuc.Text);
            }
            //delete column
            if (e.ColumnIndex == 7)
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa không?", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2) != System.Windows.Forms.DialogResult.Yes)
                    return;
                SqlCommand cmd2 = new SqlCommand("DELETE FROM [dbo].[RPBM5] WHERE gid = " + id_kqks, cn);
                int temp = 0;
        ...
        {
            UtilsDatabase.DeleteMemberByTablename(table_name);
            FrmThemmoiRPBM5 frm = new FrmThemmoiRPBM5(0);
            frm.Text = "THÊM MỚI BIÊN BẢN BÀN GIAO MẶT BẰNG CHO THI CÔNG";
            frm.ShowDialog();
            LoadData(TxtTImkiem.Text, timeNgaybatdau.Text, timeNgayketthuc.Text);
        }
    }
