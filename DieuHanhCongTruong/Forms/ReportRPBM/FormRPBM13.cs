...
{
    public partial class FormRPBM13 : Form
    {
        ...
        private void LoadData(string name, string date1, string date2)
        {
            try
            {
                ...
                sqlAdapter = new SqlDataAdapter(string.Format("select gid as 'ID', cecm_program_idST as 'Dự án', datesST as 'Thời gian', address as 'Địa điểm' from [dbo].[RPBM13] where cecm_program_idST LIKE N'%" + name + "%' and datesST >'" + date1 + "' and datesST <'" + date2 + "' order by gid"), cn);
        ...
        private void FormRPBM13_Load(object sender, EventArgs e)
        {
            SqlCommandBuilder sqlCommand = null;
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 4)
            {
                ...
                string sourceFile = AppUtils.GetAppDataPath() + "\\TempExcel\\BB_RPBM13.xls";
                string pathFile = "";
                saveFileDialog1.FileName = "RPBM13_" + Guid.NewGuid().ToString();
                ...
                if (pathFile != null)
                {
                    using (ExcelEngine excelEngine = new ExcelEngine())
                    {
                        ...
                        SqlDataAdapter sqlAdapterProvince = new SqlDataAdapter(
                            ...
                            "FROM RPBM13 " +
                            "left join cecm_provinces Tinh on RPBM13.province_id = Tinh.id " +
                            "left join cecm_programData on cecm_programData.id = RPBM13.cecm_program_id " +
                        ...
                        if (datatable.Rows.Count != 0)
                        {
                            ...
                            marker.AddVariable("num_economic_contracts", "- Căn cứ Hợp đồng kinh tế số " + dr["num_economic_contracts"].ToString() + "/HĐKT-RPBM , " + dr["date_economic_contractsST"].ToString() + " được ký kết giữa " + dr["organization_signed"].ToString());
                            marker.AddVariable("time_signedST", "Hôm nay " + dr["time_signedST"].ToString() + " tại hiện trường mặt bằng dự án: " + dr["cecm_program_idST"].ToString());

                            marker.AddVariable("area_rapha", "a) Diện tích đã rà phá bom mìn, vật nổ tại mặt bằng công trình: " + dr["area_rapha"].ToString() + " ha. Chi tiết tại bản vẽ hoàn công rà phá bom mìn kèm theo.");
                            marker.AddVariable("deep_rpbm", "b) Độ sâu rà phá bom mìn vật nổ đến " + dr["deep_rpbm"].ToString() + " và các yêu cầu khác nếu có.");
                            marker.AddVariable("noidung", "Các bên tiến hành giao, nhận mặt bằng tại thực địa phục vụ công tác thi công rà phá bom mìn vật nổ công trình: " + dr["cecm_program_idST"].ToString() + " địa điểm " + dr["address_cecm"].ToString());
   
                            marker.AddVariable("amount", "Biên bản được lập thành " + dr["amount"].ToString() + " bản có giá trị như nhau để làm cơ sở cho việc lập hồ sơ hoàn công và thanh quyết toán./.");
                        }
                        ...
                        sqlAdapterSub3 = new SqlDataAdapter(string.Format("select long5, N'Ông' as Ong, N'Chức vụ' as ChucVu, CONCAT(long1ST, string1) as string1, CONCAT(long2ST, string2) as string2 from groundDeliveryRecordsMember where (main_id = {0} and field_name = N'{1}' and table_name = N'{2}')", id_kqks, field_name2, table_name), cn);
                        ...
                        sqlAdapterSub4 = new SqlDataAdapter(string.Format("select long5, N'Ông' as Ong, N'Chức vụ' as ChucVu, CONCAT(long1ST, string1) as string1, CONCAT(long2ST, string2) as string2 from groundDeliveryRecordsMember where (main_id = {0} and field_name = N'{1}' and table_name = N'{2}')", id_kqks, field_name4, table_name), cn);
                        ...
                        bool coNguoiKy4 = false;
                        foreach (DataRow drCDT in datatableSub4.Rows)
                        {
                            if (drCDT["long5"].ToString() == "1")
                            {
                                marker.AddVariable("CQDP", drCDT["string1"].ToString());
                                coNguoiKy4 = true;
                                break;
                            }
                        }
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                SqlDataAdapter sqlAdapterProvince = new SqlDataAdapter("SELECT * , cecm_programData.code as 'cecm_program_code'  FROM RPBM13 left join cecm_programData on cecm_programData.id = RPBM13.cecm_program_id where gid = " + id_kqks, cn);
                ...
                {
                    classRPBM13 lst = new classRPBM13();
                    ...
                    lst.area_cecm = double.Parse(dr["area_cecm"].ToString());
                    ...
                    lst.local_id = int.Parse(dr["local_id"].ToString());
                    try
                    {
                    ...
                    lst.reportHandoverResult_CDTMember_lstSubTable = Lst1;
                    lst.reportHandoverResult_ConsMem_lstSubTable = Lst2;
                    lst.reportHandoverResult_SurveyMem_lstSubTable = Lst3;
                    lst.reportHandoverResult_LocalMem_lstSubTable = Lst4;

                    string json = JsonConvert.SerializeObject(lst, Formatting.Indented);

                    var saveLocation = AppUtils.SaveFileJson("JsonRPBM13");
            ...
            if (e.ColumnIndex == 6)
            {
                ...
                FrmThemmoiRPBM13 frm = new FrmThemmoiRPBM13(id_kqks);
            ...
            if (e.ColumnIndex == 7)
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa không?", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2) != System.Windows.Forms.DialogResult.Yes)
                    return;
                SqlCommand cmd2 = new SqlCommand("DELETE FROM [dbo].[RPBM13] WHERE gid = " + id_kqks, cn);
                int temp = 0;
        ...
        {
            UtilsDatabase.DeleteMemberByTablename(table_name);
            FrmThemmoiRPBM13 frm = new FrmThemmoiRPBM13(0);
